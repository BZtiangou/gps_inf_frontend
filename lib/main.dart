...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<String> getDeviceInfo() async {
    ...
    try {
      ...
      if (Platform.isAndroid) {
        ...
        deviceInfo = '${androidInfo.brand} ${androidInfo.model} OS Version: ${androidInfo.version.release}';
      } else if (Platform.isIOS) {
        IosDeviceInfo iosInfo = await deviceInfoPlugin.iosInfo;
        deviceInfo = '${iosInfo.name} ${iosInfo.model} OS Version: ${iosInfo.systemVersion}';
      }
    } catch (e) {
  ...
  Future<void> collectAndSendACCData([String? deviceInfo]) async {
    ...
    final response = await http.post(
      ...
      body: data,
      //因为已经包含了设备型号和设备信息(相当于用户的身份证)，所以没有进行二次验证
      // headers: {

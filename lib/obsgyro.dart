...
Future<void> fetchAndDisplayACCData(List<List<double>> dataPoints, List<String> dt) async {
  ...
  var url = Uri.parse('http://gps.primedigitaltech.com:8000/sensor/getGyrodata/');
  try {
    ...
    if (response.statusCode == 200) {
      ...
      var accelerometers = jsonData['gyro'];
      for (var i = 0; i < 100; i++) {
...
class GyroObservationPage extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MyHomePage(title: 'gyro:'),
    );
  }
}
      ...
      body: Center(
        child: Column(
          children: <Widget>[
            ...
            const Text('Gyro: information used to calculate user status'),
          ],
        ),
      ),
